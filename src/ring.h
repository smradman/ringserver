...
#define LEGACY_MSEED_STREAMID_PATTERN \
  "^[0-9A-Z]{1,2}_[0-9A-Z]{1,5}_[0-9A-Z]{0,2}_[0-9A-Z]{3}/MSEED$"

/* Macros for updating different patterns */
#define RingAllowed(reader, pattern) \
  UpdatePattern (&(reader)->allowed, &(reader)->allowed_data, pattern, "ring allowed")
#define RingForbidden(reader, pattern) \
  UpdatePattern (&(reader)->forbidden, &(reader)->forbidden_data, pattern, "ring forbidden")
#define RingMatch(reader, pattern) \
  UpdatePattern (&(reader)->match, &(reader)->match_data, pattern, "ring match")
#define RingReject(reader, pattern) \
  UpdatePattern (&(reader)->reject, &(reader)->reject_data, pattern, "ring reject")
...
extern uint64_t RingRead (RingReader *reader, uint64_t reqid, RingPacket *packet, char *packetdata);
...
extern int UpdatePattern (pcre2_code **code, pcre2_match_data **data, const char *pattern,
...
extern int RingStreamAllowed (RingReader *reader, const char *streamid);
